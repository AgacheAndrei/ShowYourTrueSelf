...
def eye_closed(landmarks):
    ...
    ear_threshold = 0.17  # Adjust this threshold as needed
...
def visualize(image, detection_result, face_landmarks_result):
    ...
    if detection_result.detections:
        ...
        if face_landmarks_result.multi_face_landmarks:
...
# Variable to track interruption
interrupted = False
...
while True:
    ...
    if face_counter > 1:
        print("Fail : More than one face detected! Closing the video.",param1)
        interrupted = True
        break
    elif face_landmarks_result.multi_face_landmarks:
        # Assuming the first detected face
        first_face_landmarks = face_landmarks_result.multi_face_landmarks[0]

        if eye_closed(first_face_landmarks.landmark):
            elapsed_time = time.time() - start_time
            if elapsed_time > 3:
                print("Fail : Eyes closed for more than 3 seconds! Closing the video.",param1)
                interrupted = True
                break
            else:
                start_time = time.time()
    else:
        print("Fail : No face landmarks detected.",param1)
        interrupted = True
        break  # Assuming this is the appropriate condition for ending the loop
...
if not interrupted:
    print("Success : Test completed successfully with no interruptions.",param1)
